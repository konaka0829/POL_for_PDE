...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--nu", type=float, default=0.1, help="Heat semigroup diffusion coefficient.")
...
def _sg_loss_1d(z_enc: torch.Tensor, z_ref: torch.Tensor) -> torch.Tensor:
    # Sum over batch of per-sample mean squared error in latent space.
    return ((z_enc - z_ref) ** 2).mean(dim=(1, 2)).sum()
    ...
    for ep in range(args.epochs):
        ...
        for (u_batch,) in train_loader:
            ...
            for n in range(1, args.T + 1):
                ...
                sg_acc = sg_acc + _sg_loss_1d(z_enc, z)
