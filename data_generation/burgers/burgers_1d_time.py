"""Generate 1D viscous Burgers trajectories (time-last) in MAT format.

Output:
  u: [N, S, T_total]  (time-last)
  t: [T_total]
"""
...
# Reuse GaussianRF from existing Navier-Stokes data generator.
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="1D viscous Burgers time-series generator")
    p.add_argument("--out", type=str, default="data/burgers_1d_ts.mat", help="Output .mat file path")
    p.add_argument("--N", type=int, default=1000, help="Number of trajectories")
    ...
    p.add_argument("--dt", type=float, default=1e-4, help="Internal time step")
    p.add_argument("--record-steps", type=int, default=200, help="Number of snapshots")
    p.add_argument("--nu", type=float, default=1e-2, help="Viscosity")
...
def _integrate_burgers(u0: np.ndarray, t_final: float, dt: float, record_steps: int, nu: float) -> tuple[np.ndarray, np.ndarray]:
    s = u0.shape[0]
    ...
    k2 = k * k
    kmax = s // 2
    dealias = (np.abs(np.fft.fftfreq(s, d=1.0 / s)) <= (2.0 / 3.0) * kmax).astype(np.float64)
...
def main() -> None:
    ...
    if args.N <= 0 or args.S <= 1 or args.record_steps <= 1 or args.dt <= 0 or args.T_final <= 0 or args.nu <= 0:
    ...
    for i in range(args.N):
        ...
        u0 *= float(args.scale)
        traj, t = _integrate_burgers(u0=u0, t_final=args.T_final, dt=args.dt, record_steps=args.record_steps, nu=args.nu)
        u[i] = traj
        if t_ref is None:
            t_ref = t
