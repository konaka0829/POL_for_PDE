...
if sys.version_info < (3, 10):
    sys.stderr.write(
        "model1_error_decomposition_1d.py requires Python 3.10+.\n"
        "You are running Python %s.\n"
        "Use `python3 model1_error_decomposition_1d.py ...` instead.\n" % sys.version.split()[0]
    )
    raise SystemExit(1)
...
def _build_ttilde_range(start, stop, step):
    if step <= 0.0:
...
def build_parser():
    ...
    parser.add_argument(
        "--beta-mode",
        choices=("zero", "analytic_safe", "analytic_safe_poincare", "empirical_pairwise", "fixed"),
        default="zero",
    )
    parser.add_argument("--beta-fixed", type=float, default=0.0)
    parser.add_argument("--beta-pairwise-margin", type=float, default=0.0)
    parser.add_argument("--beta-max-states", type=int, default=24)
    parser.add_argument("--calibration-num-samples", type=int, default=0)
    parser.add_argument("--calibration-seed", type=int, default=None)
    parser.add_argument("--time-quadrature", choices=("trapezoid", "left"), default="trapezoid")
