...
def test_cli_rejects_stale_png_without_overwrite(tmp_path):
    ...
    p=subprocess.run([sys.executable,"tests/paper1_recipe_driver.py","e1","--config","configs/paper1_e1_smoke.json","--e0-dir",str(tmp_path/"missing"),"--output-dir",str(out)],capture_output=True,text=True)
    assert p.returncode!=0 and "nonempty" in p.stderr

@pytest.fixture
def e0_dir(tmp_path):
    out=tmp_path/"e0"
    process=subprocess.run([sys.executable,"tests/paper1_recipe_driver.py","e0","--config","configs/paper1_e0_for_e1_smoke.json","--output-dir",str(out),"--overwrite"],capture_output=True,text=True)
...
def test_cli_gate_and_saved_qa_fail_nonzero(e0_dir,tmp_path,mutation,reason):
    ...
    process=subprocess.run([sys.executable,"tests/paper1_recipe_driver.py","e1","--config",str(config),"--e0-dir",str(e0_dir),"--output-dir",str(out),"--overwrite","--skip-plots","--torch-threads","1"],capture_output=True,text=True)
