"""Fixed random-feature map for Paper 1 Model 3."""
...
class RandomFeatureMap:
    ...
    @classmethod
    def create(
        cls, J: int, width: int, *, activation: str, seed: int,
        weight_scale: float, bias_scale: float, dtype: torch.dtype, device: torch.device | str,
    ) -> "RandomFeatureMap":
        if J <= 0 or width <= 0:
        ...
        A = weight_scale * torch.randn(width, J, generator=generator, dtype=dtype)
        c = bias_scale * torch.randn(width, generator=generator, dtype=dtype)
        return cls(A.to(device), c.to(device), activation, int(seed), float(weight_scale), float(bias_scale))
