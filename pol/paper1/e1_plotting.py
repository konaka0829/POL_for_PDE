...
def create_e1_plots(out: Path, tables: dict) -> list[dict]:
    ...
    for regime in ("stable", "unstable"):
        ...
        name=f"multiplier_{regime}.png"; fig.savefig(out/name,dpi=160,bbox_inches="tight"); plt.close(fig); made.append({"relative_path":name,"status":"created","source":"mode_comparison.csv"})
    fig, ax=plt.subplots()
    ...
    ax.set_yscale("log"); ax.legend(); ax.set_xlabel("q"); ax.set_ylabel("relative L2"); fig.savefig(out/"bandwidth_error.png",dpi=160,bbox_inches="tight"); plt.close(fig); made.append({"relative_path":"bandwidth_error.png","status":"created","source":"selected_results.csv"})
    fig, ax=plt.subplots()
    ...
    ax.set_yscale("log"); ax.legend(); ax.set_xlabel("q"); ax.set_ylabel("operator norm"); fig.savefig(out/"readout_operator_norm.png",dpi=160,bbox_inches="tight"); plt.close(fig); made.append({"relative_path":"readout_operator_norm.png","status":"created","source":"readout_diagnostics.csv"})
    fig, ax=plt.subplots()
    ...
    ax.legend(); ax.set_xlabel("noise level"); ax.set_ylabel("output perturbation RMS"); fig.savefig(out/"noise_sensitivity.png",dpi=160,bbox_inches="tight"); plt.close(fig); made.append({"relative_path":"noise_sensitivity.png","status":"created","source":"noise_summary.csv"})
    return made
