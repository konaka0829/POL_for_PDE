...
from pol.paper1.e2_qa import (
    CSV_CONTRACT_VERSION, assert_finite, expected_artifacts, validate_artifact_contract,
    validate_csv, validate_result_cartesian, validate_resume_output,
    write_manifest,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        plots = []
        make_plots = result["test_evaluated"] and not args.skip_plots
        if make_plots:
    ...
    expected = expected_artifacts(
        status=summary["status"], skip_plots=args.skip_plots,
        test_evaluated=bool(summary["test_evaluated"]),
        failure_kind=summary.get("failure_kind"))
    validate_artifact_contract(
        stage, status=summary["status"], skip_plots=args.skip_plots,
        test_evaluated=bool(summary["test_evaluated"]),
