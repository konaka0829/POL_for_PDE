...
def build_default_nu_values() -> list[float]:
    values = [digit * 1e-3 for digit in range(1, 10)]
    values.extend(digit * 1e-2 for digit in range(1, 10))
    values.append(1e-1)
    return values


def parse_nu_values(raw: str) -> list[float]:
    if not raw.strip():
        return build_default_nu_values()
    values = [float(item) for item in parse_csv_values(raw)]
    if not values:
        raise ValueError("No nu values were parsed")
    for value in values:
        if value <= 0.0:
            raise ValueError("nu values must be positive")
    return values
...
def build_run_command(args: argparse.Namespace, model: str, nu: float, out_dir: Path) -> list[str]:
    cmd = [
        args.python,
...
def save_model_plot(model: str, rows: list[dict[str, Any]], out_path: Path) -> None:
    ...
    ax.loglog(
        ...
        [row["test_relL2_plot"] for row in rows],
        marker="o",
        linewidth=1.6,
        label=model,
    )
    ...
    ax.set_title("%s: nu vs error" % model)
...
def save_combined_plot(model_rows: dict[str, list[dict[str, Any]]], out_path: Path) -> None:
    ...
    ax.set_xlabel("nu")
    ax.set_ylabel("test relL2")
    ax.set_title("Model 1/2/3: nu vs error")
    ax.grid(True, which="both", linestyle="--", alpha=0.35)
    ax.legend()
    fig.tight_layout()
    for ext in ("png", "pdf", "svg"):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep res-burgers-nu for model123_burgers_1d.py and save log-log error plots."
    )
    parser.add_argument("--python", default=sys.executable)
    parser.add_argument("--models", default="model1,model2,model3")
    parser.add_argument("--nu-values", default="")
    parser.add_argument("--data-file", default="data/burgers_T10_nu001.mat")
    parser.add_argument("--out-root", default="outputs/model123_burgers_nu_sweep")
...
def validate_args(args: argparse.Namespace) -> tuple[list[str], list[float]]:
    ...
    nu_values = parse_nu_values(args.nu_values)
    return models, nu_values
...
def run_model_jobs(
    *,
    ...
    for nu in nu_values:
        run_dir = model_dir / ("nu_" + safe_tag(nu))
        ...
        if args.skip_existing and config_path.exists():
            ...
            rows.append(row)
            print("[%s] nu=%g -> reuse test=%.6e" % (model, nu, row["test_relL2"]), flush=True)
            continue

        if args.dry_run:
            cmd = build_run_command(args, model, nu, run_dir)
            run_one(cmd, run_dir / "stdout_stderr.log", env=env, dry_run=True)
            row["status"] = "dry_run"
            row["return_code"] = 0
            rows.append(row)
            print("[%s] nu=%g -> dry-run" % (model, nu), flush=True)
            continue
    ...
    if args.dry_run or not pending_jobs:
        rows.sort(key=lambda item: float(item["nu"]))
    ...
    while pending_idx < total_jobs or running_jobs:
        ...
        for job in running_jobs:
            ...
            if return_code == 0 and config_path.exists():
                ...
                row["test_relL2_plot"] = clip_for_log(float(row["test_relL2"]), eps)
                print(
                    "[done %d/%d] [%s] nu=%g -> test=%.6e"
                    % (completed, total_jobs, model, job["nu"], row["test_relL2"]),
                    flush=True,
                )
            else:
                row["status"] = "fail"
                had_failure = True
                print(
                    "[fail %d/%d] [%s] nu=%g -> rc=%s"
                    % (completed, total_jobs, model, job["nu"], return_code),
                    flush=True,
                )
            rows.append(row)
...
def main() -> int:
    ...
    models, nu_values = validate_args(args)
    ...
    for model in models:
        ...
        if ok_rows:
            save_model_plot(model, ok_rows, model_dir / "nu_vs_error_loglog")
            model_rows[model] = ok_rows

    if model_rows:
        combined_rows = {model: rows for model, rows in model_rows.items() if rows}
        if combined_rows:
            save_combined_plot(combined_rows, out_root / "nu_vs_error_loglog_all_models")
